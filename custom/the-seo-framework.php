...
add_filter('the_seo_framework_description_output', 'bogopx_override_seo_framework_home_description', 10);
 ...
 */
function bogopx_override_seo_framework_home_description ($desc) {
  // If Bogo installed, not default locale, and static front page
  if (class_exists('Bogo') && !Bogo::is_default_locale() && is_front_page() && !is_home()) {
    global $post;
    if (!$post) { return $desc; }
